    ...
    <div className="min-h-screen bg-background text-on-background pb-28">
      <header className="fixed top-0 z-50 w-full bg-[#131313] border-b border-outline/10">
        <div className="h-16 px-6 flex items-center gap-2 max-w-2xl mx-auto">
          ...
          <h1 className="font-display font-black text-xl italic uppercase text-primary">FORGE <span className="text-on-surface">/ RACE</span></h1>
        </div>
      </header>
      <main className="pt-24 px-6 max-w-2xl mx-auto space-y-6">
        <section className="bg-surface-container border border-outline/20 rounded-xl p-4 space-y-3">
          ...
          <h2 className="font-display font-black text-3xl uppercase italic">Race Prep Planner</h2>
          ...
          <input value={manualWallBalls} onChange={(e) => setManualWallBalls(e.target.value.replace(/[^0-9]/g, ""))} placeholder="Manual Wall Balls seconds (optional)" className="w-full bg-surface rounded-lg p-3 text-xs" />
          <button onClick={() => addRacePlan(plan)} className="w-full bg-primary text-on-primary rounded-xl p-4 font-display font-black uppercase text-xs flex items-center justify-center gap-2"><Save className="w-4 h-4" /> Save Race Plan</button>
        </section>
        <section className="grid grid-cols-2 gap-3">
          <div className="bg-surface-container rounded-xl p-4 border border-outline/20">
            <p className="text-[10px] text-outline uppercase">Projected</p>
            <p className="font-mono text-2xl font-black text-primary">{msToClock(plan.projectedTimeMs)}</p>
          </div>
          <div className="bg-surface-container rounded-xl p-4 border border-outline/20">
            <p className="text-[10px] text-outline uppercase">ROXZONE</p>
        ...
        <section className="bg-surface-container border border-outline/20 rounded-xl p-4">
          <h3 className="font-display font-black uppercase text-lg mb-3">Run Splits</h3>
        ...
        <section className="bg-surface-container border border-outline/20 rounded-xl p-4">
          <h3 className="font-display font-black uppercase text-lg mb-3">Station Splits</h3>
        ...
        {racePlans.length > 0 && <p className="text-xs text-outline">{racePlans.length} saved race plan(s) in this browser.</p>}
      </main>
      <BottomNavBar />
    </div>
  );
